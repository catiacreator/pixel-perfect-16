...
const PRODUTOS = [
  {
    key: "conteudo-ia",
    tag: "Mini-curso",
    titulo: "Primeiro Mês de Posts",
    assinatura: "criar conteúdo com IA",
  ...
  const renderCard = (p: (typeof PRODUTOS)[number], i: number) => {
    ...
    return (
      <Wrapper
        key={p.key}
        {...wrapperProps}
        className={`fade-up group relative overflow-hidden rounded-[24px] border border-white/60 flex flex-col justify-end aspect-[9/16] w-full sm:w-[330px] md:w-[360px] shrink-0 p-6 text-left transition-all duration-300 ${locked && !modoDesbloquear ? "cursor-not-allowed" : "hover:-translate-y-1.5 hover:shadow-[0_34px_70px_-30px_rgba(40,20,15,0.55)]"}`}
